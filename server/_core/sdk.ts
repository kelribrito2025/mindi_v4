...
class SDKServer {
  ...
  private getCookieValues(cookieHeader: string | undefined, name: string): string[] {
    ...
    const parsedValue = this.parseCookies(cookieHeader).get(name);
    if (parsedValue && !values.includes(parsedValue)) {
  ...
  async authenticateRequest(req: Request): Promise<User> {
    // Regular authentication flow. Browsers can send duplicate cookies
    // after a domain/secret migration (for example, one legacy domain cookie and
    // one new host-only cookie). Try every unique candidate and keep the last
    // valid session so a fresh login wins over any still-valid legacy cookie.
    const sessionCookies = this.getCookieValues(req.headers.cookie, COOKIE_NAME);
    let sessionCookie: string | undefined;
    let session: SessionPayload | null = null;

    for (const candidate of sessionCookies) {
      const verified = await this.verifySession(candidate);
      if (verified) {
        // When browsers send duplicated cookies with the same name after a
        // domain/path migration, the newest host-only cookie is commonly sent
        // after the older domain cookie. Prefer the last valid candidate so a
        // fresh login replaces any still-valid legacy session.
    ...
    if (!session) {
      if (sessionCookies.length === 0) {
        await this.verifySession(undefined);
      }
