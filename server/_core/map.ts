...
export async function makeRequest<T = unknown>(
  ...
  // Construct full URL. With a dedicated Google Maps key, call Google directly;
  // otherwise keep the legacy Forge/Manus proxy path.
  const url = new URL(useProxy ? `${baseUrl}/v1/maps/proxy${endpoint}` : `${baseUrl}${endpoint}`);
