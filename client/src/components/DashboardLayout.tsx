...
function DashboardLayoutContent({
  ...
  // Auto-expand/collapse sidebar baseado na largura da janela (apenas desktop)
  // Abaixo de 1280px (xl) colapsa automaticamente, acima expande
  const autoResizeRef = useRef(true); // flag para distinguir auto vs manual
  useEffect(() => {
    if (isMobile) return;
    const COLLAPSE_BREAKPOINT = 1280;
    const handleResize = () => {
      const width = window.innerWidth;
      if (width < COLLAPSE_BREAKPOINT && open) {
        autoResizeRef.current = true;
        setOpen(false);
      } else if (width >= COLLAPSE_BREAKPOINT && !open && autoResizeRef.current) {
        setOpen(true);
      }
    };
